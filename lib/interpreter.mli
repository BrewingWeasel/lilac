...
type context = {
  patterns :
    ...
    * Ast.pattern Location.located_span)
    StringMap.t;
  functions :
    (string Location.located_span * Ast.function_) list
    StringMap.t;
  scopes : Scope.scope list;
}

val context_from_definitions :
  Ast.definition Location.located_span list -> context
...
val eval :
  context -> Ast.expression -> (Value.value, error) result
